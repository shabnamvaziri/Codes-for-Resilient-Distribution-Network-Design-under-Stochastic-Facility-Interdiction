	...
	if (sumy == J){
		...
		for (iter1 = 0; iter1 < max_iter1; iter1++){
			...
			for (sGlobal = 0; sGlobal < S; sGlobal++){
				...
				for (j = 0; j < J + 1; j++){
					xxxx[tempi] += p[sGlobal] * (alpha1[j] * y[j] * epsilon[(j*S + sGlobal)] - alpha1[j] * y[j] * xi[j*S + sGlobal] * Delta1[j*S + sGlobal]);
				}
		...
		if (counter != J*S){
			...
			for (fillcounter = 0; fillcounter < filled; fillcounter++){
				...
				for (sGlobal = 0; sGlobal < S; sGlobal++){
					...
					for (j = 0; j < J + 1; j++){
						xxxx[sGlobal] += p[sGlobal] * (alpha1[j] * y[j] * epsilon[(j*S + sGlobal)] - alpha1[j] * y[j] * xi[j*S + sGlobal] * Delta1[(j*S + sGlobal)]);
					}
				...
				if (Qcurrent > currentLB){
					...
					for (sGlobal = 0; sGlobal < S; sGlobal++){
						...
						for (j = 0; j < J; j++){
							epsilon_Keep[(j*S + sGlobal) + ((iter)*(J + 1)*S)] = epsilon[(j*S + sGlobal)];
							Delta1_Keep[(j*S + sGlobal) + ((iter)*(J + 1)*S)] = Delta1[(j*S + sGlobal)];
...
printf("sub4\n");
		...
		for (iter1 = 0; iter1 < max_iter1; iter1++){
			...
			for (tempi = 0; tempi < S2; tempi++){
				...
				for (j = 0; j < J + 1; j++){
					xxxx[tempi] += prob[tempi*max_iter + iter] * (alpha1[j] * y[j] * epsilon[(j*S + selectedscenario[tempi])] - alpha1[j] * y[j] * xi[j*S + selectedscenario[tempi]] * Delta1[j*S + selectedscenario[tempi]]);
				}
				Q += (xx[tempi] - xxx[tempi] - xxxx[tempi]);
			}
printf("sub3\n");
...
printf("sub9\n");
								for (j = 0; j < J; j++){
									epsilon_Keep[(j*S + sGlobal) + ((iter)*(J + 1)*S)] = epsilon[(j*S + selectedscenario[tempi])];
									Delta1_Keep[(j*S + sGlobal) + ((iter)*(J + 1)*S)] = Delta1[(j*S + selectedscenario[tempi])];

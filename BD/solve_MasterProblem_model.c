...
double solve_MasterProblem(void)
{
	...
	if (iter > 0){
		...
		for (ii = 0; ii < iter; ii++){
			...
			for (j = 0; j < J; j++){
				for (s = 0; s < S; s++){
					sumed[j] = sumed[j] + epsilon[(j*S + s) + (ii*(J + 1)*S)] - Delta1[(j*S + s) + (ii*(J + 1)*S)];
				}
				if (sumed[j] > 0)
			...
			for (j = 0; j < J; j++){
				for (s = 0; s < S; s++){
					temp3[j*max_iter + ii] += epsilon[(j*S + s) + (ii*(J + 1)*S)] * p[s] * alpha1[j];
				}
			}
			for (j = 0; j < J; j++){
				for (s = 0; s < S; s++){
					temp4[j*max_iter + ii] += p[s] * Delta1[(j*S + s) + (ii*(J + 1)*S)] * xi[j * S + s] * alpha1[j];
				}
			}

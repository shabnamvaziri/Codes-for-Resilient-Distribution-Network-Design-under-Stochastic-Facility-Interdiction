...
double solve_SubProblem(void)
{
	...
	double      num_beta_var, num_delta_var, num_epsilon_var, num_gamma_var, num_Delta1_var;
	double      *pos_beta, *pos_delta, *pos_epsilon, *pos_gamma, *pos_Delta1, *M;
	int auxilary = 0;
	int auxilary1 = 0;
	...
	pos_epsilon = create_double_vector((J + 1)* S);
	pos_gamma = create_double_vector((J + 1)* S);
	pos_Delta1 = create_double_vector((J + 1)* S);
	...
	for (j = 0; j < J + 1; j++){
		for (s = 0; s < S; s++){
			//	sprintf(colname[(int)(index1 + num_x_var + num_beta_var + num_delta_var)], "epsilon%3d_%3d", j, s);
			pos_epsilon[j*S + s] = index1 + num_x_var + num_beta_var + num_delta_var;
			obj[index1] = -1 * (p[s] * alpha1[j] * y[j]) - 0.0000001;
			//obj[index1] = -1 * (p[s] * alpha1[j] * y[j]) ;
			ctype[index1] = 'C';
			lb[index1] = 0;
			ub[index1] = CPX_INFBOUND;
			index1++;
	...
	for (j = 0; j < J + 1; j++){
		for (s = 0; s < S; s++){
			//sprintf(colname[(int)(index1 + num_x_var + num_beta_var + num_delta_var + num_epsilon_var)], "gamma%3d_%3d", j, s);
			pos_gamma[j*S + s] = index1 + num_x_var + num_beta_var + num_delta_var + num_epsilon_var;
			obj[index1] = 0;
			ctype[index1] = 'C';
			lb[index1] = 0;
			ub[index1] = CPX_INFBOUND;
	...
	for (j = 0; j < J + 1; j++){
		for (s = 0; s < S; s++){
			...
			pos_Delta1[j*S + s] = index1 + num_x_var + num_beta_var + num_delta_var + num_epsilon_var + num_gamma_var;
			obj[index1] = p[s] * alpha1[j] * y[j] * xi[j*S + s];
			ctype[index1] = 'C';
			lb[index1] = 0;
			ub[index1] = CPX_INFBOUND;
			index1++;
	...
	for (i = 0; i < I; i++){
		for (j = 0; j < J + 1; j++){
			for (s = 0; s < S; s++){
				...
				matval[index++] = -1;
				matind[index] = pos_epsilon[j*S + s];
				matval[index++] = -1;
			}
		}
	}
...
if (B>1)
	...
	for (j = 0; j < J + 1; j++){
		for (s = 0; s < S; s++){
			epsilon[(j*S + s) + (iter*(J + 1)*S)] = x[index];
			if (epsilon[(j*S + s) + (iter*(J + 1)*S)] > 0){
				sumepsi += epsilon[(j*S + s) + (iter*(J + 1)*S)];
			}
			index++;
		}
	}
	...
	for (j = 0; j < J + 1; j++){
		for (s = 0; s < S; s++){
			Delta1[(j*S + s) + (iter*(J + 1)*S)] = x[index];
			index++;
		}
	}

...
double solve_MasterProblem(void)
{
	...
	int		  num_v_var, num_vPerim_var, num_y_var, num_theta_var;
	int	    pos_theta;
	int    *pos_v, *pos_vPerim;
	int       *pos_y;
	double tempV = 0;
	int jcounter = 0;
	int ii = 0;
	...
	pos_vPerim = create_int_vector(I* J);
	...
	if (iter > 0){
		for (ii = 0; ii < iter; ii++){
			...
			for (j = 0; j < J; j++){
				for (s = 0; s < S; s++){
					sumed[j] = sumed[j] + epsilon_Keep[(j*S + s) + (ii*(J + 1)*S)] - Delta1_Keep[(j*S + s) + (ii*(J + 1)*S)];
				}
				if (sumed[j] > 0)
			...
			for (k = 0; k < K; k++){
				for (s = 0; s < S; s++){
					temp1 += beta_Keep[(k*S + s) + (ii*S*K)] * p[s] * d[k];
			...
			for (i = 0; i < I; i++){
				for (s = 0; s < S; s++){
					temp2 += delta_Keep[(i*S + s) + (ii*I*S)] * p[s] * alpha[i];
				}
			}
			for (j = 0; j < J; j++){
				for (s = 0; s < S; s++){
					temp3[j*max_iter + ii] += epsilon_Keep[(j*S + s) + (ii*(J + 1)*S)] * p[s] * alpha1[j];
				}
			}
			for (j = 0; j < J; j++){
				for (s = 0; s < S; s++){
					temp4[j*max_iter + ii] += p[s] * Delta1_Keep[(j*S + s) + (ii*(J + 1)*S)] * xi[j * S + s] * alpha1[j];
				}
			}
			...
			for (j = 0; j < J; j++){
				matind[index] = pos_y[j];
				matval[index++] = temp3[j*max_iter + ii] - temp4[j*max_iter + ii];
			}
			status = CPXaddrows(env, lp, 0, index1, index, rhs, sense, matbeg, matind, matval, NULL, NULL);
			if (status)
				fprintf(stderr, "CPXaddrows failed.\n");
	...
	//CPXwriteprob(env, lp, "model1.lp", NULL);                          //write the model in .lp format if needed (to debug)
	...
	CPXsetdblparam(env, CPX_PARAM_NODEFILEIND, 0);
	...
	for (j = 0; j < J; j++){
		if (y[j] == 1){
			//	openFacility[jCounter] = j;
			jPerim[iter] ++;
			openFacility[jcounter*max_iter + iter] = j;
			jcounter++;
		}
	}
	////finish adding

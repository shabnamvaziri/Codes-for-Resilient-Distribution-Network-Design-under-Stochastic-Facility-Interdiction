...
double LowerBound1(void){
	...
	pos_Delta1 = create_int_vector((J + 1)* S);
	...
	for (j = 0; j < J + 1; j++){
		for (sGlobal = 0; sGlobal < S; sGlobal++){
			...
			pos_Delta1[j*S + sGlobal] = index1 + num_x_var;
			obj[index1] = p[sGlobal] * alpha1[j] * y[j] * xi[j*S + sGlobal];
			ctype[index1] = 'C';
			lb[index1] = 0;
	...
	for (j = 0; j < J + 1; j++){
		for (sGlobal = 0; sGlobal < S; sGlobal++){
			...
			rhs[index1] = 0;
			matbeg[index1++] = index;
			matind[index] = pos_Delta1[j*S + sGlobal];
			matval[index++] = 1;
			matind[index] = pos_x[j*S + sGlobal];
	...
	for (j = 0; j < J + 1; j++){
		for (sGlobal = 0; sGlobal < S; sGlobal++){
			sense[index1] = 'L';
			rhs[index1] = epsilon[(j * S + sGlobal)];
			matbeg[index1++] = index;
			matind[index] = pos_Delta1[j * S + sGlobal];
			matval[index++] = 1;
		}
	...
	for (j = 0; j < J + 1; j++){
		for (sGlobal = 0; sGlobal < S; sGlobal++){
			sense[index1] = 'G';
			rhs[index1] = -MAX_DOUBLE2 + epsilon[(j*S +sGlobal)];
			matbeg[index1++] = index;
			matind[index] = pos_Delta1[j*S + sGlobal];
			matval[index++] = 1;
			matind[index] = pos_x[j*S + sGlobal];
	...
	for (sGlobal = 0; sGlobal < S; sGlobal++){
		...
		for (j = 0; j < J + 1; j++){
			calculatepsilon[sGlobal] += p[sGlobal] * alpha1[j] * y[j] * epsilon[(j*S + sGlobal)];
		}
		Qcalculate += (calculatebeta[sGlobal] - calculatedelta[sGlobal] - calculatepsilon[sGlobal]);
	}

...
double LowerBound(void){
	...
	pos_newbeta= create_int_vector((J + 1) * (J+1) * S2);
	...
	for (j = 0; j < J + 1; j++){
		for (jj = 0; jj < J + 1; jj++) {
			for (tempi = 0; tempi < S2; tempi++){
				pos_newbeta[(j * (J + 1) + jj) + (tempi * (J + 1) * (J + 1))] = index1 + num_x_var;
				obj[index1] = -1 * prob[tempi*max_iter + iter] * alpha1[j] * y[j];
				ctype[index1] = 'C';
				lb[index1] = 0;
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
			for (s = 0; s < S2; s++) {
				sense[index1] = 'G';
				rhs[index1] = rhs[index1] = (1 - correlation[j * (J + 1) + jj]) * tau[(j * (J + 1) + jj) + (s * (J + 1) * (J + 1))];
				matbeg[index1++] = index;
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
			for (s = 0; s < S2; s++) {
				sense[index1] = 'G';
				rhs[index1] = tau[(j * (J + 1) + jj) + (selectedscenario[s] * (J + 1) * (J + 1))];
				matbeg[index1++] = index;
				matind[index] = pos_newbeta[(j * (J + 1) + jj) + (s * (J + 1) * (J + 1))];
				matval[index++] = 1;
				matind[index] = pos_x[jj];
				matval[index++] = M[j] * xi[jj*S + selectedscenario[s]];
			}
		}
	}
	...
	for (tempi = 0; tempi < S2; tempi++){
		...
		Qcalculate += (calculatebeta[tempi] - calculatedelta[tempi]);
	}

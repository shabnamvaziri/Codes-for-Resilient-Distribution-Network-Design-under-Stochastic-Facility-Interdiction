...
extern double     *x1, theta, theta0, B, *b, *c, *cperim, Q, theta1, *lambda, *temps, Qcurrent, * tau, ** newbeta_Keep;
	...
	if (iter > 0){
		for (ii = 0; ii < iter; ii++){
			...
			for (j = 0; j < J; j++) {
				for (jj = 0; jj < J + 1; jj++) {
					for (s = 0; s < S; s++) {
						sumed[j] = sumed[j] + newbeta_Keep[(j * (J + 1) + jj) + (s * (J + 1) * (J + 1))][ii];
					}
				}
				if (sumed[j] > 0) {
			...
			for (j = 0; j < J; j++) {
				for (jj = 0; jj < J + 1; jj++) {
					for (s = 0; s < S; s++) {
						temp4[j * max_iter + ii] += p[s] * newbeta_Keep[(j * (J + 1) + jj) + (s * (J + 1) * (J + 1))][ii] * alpha1[j];
					}
				}
			}

...
double XCPLEX(void)
{
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
			sense[index1] = 'G';
			rhs[index1] = -1 * set[(fillcounter*(J + 1) + j)] * M[j] *  xi[jj * S + selectedscenario[tempi]]; 
			matbeg[index1++] = index;
			matind[index] = pos_newbeta[(j * (J + 1) + jj)];
			matval[index++] = 1;
			matind[index] = pos_tau[(j * (J + 1) + jj)];
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
				tau[(j * (J + 1) + jj) + (selectedscenario[tempi] * (J + 1) * (J + 1))] = x5[index];
				index++;
		}
	}
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
				newbeta[(j * (J + 1) + jj) + (selectedscenario[tempi] * (J + 1) * (J + 1))] = x5[index];

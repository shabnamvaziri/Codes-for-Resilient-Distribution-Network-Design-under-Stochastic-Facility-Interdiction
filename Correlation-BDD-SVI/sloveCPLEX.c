...
double solveCPLEX(void)
{
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
			...
			matval[index++] = M[j] * xi[jj * S + selectedscenario[tempi]];
		}
	}
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
				tau[(j * (J + 1) + jj) + (selectedscenario[tempi] * (J + 1) * (J + 1))] = x2[index];
				index++;
		}
	}
	...
	for (j = 0; j < J + 1; j++) {
		for (jj = 0; jj < J + 1; jj++) {
				newbeta[(j * (J + 1) + jj) + (selectedscenario[tempi] * (J + 1) * (J + 1))]= x2[index];
